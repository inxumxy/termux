...
function apiRequest($method, $parameters = []) {
    ...
    if (curl_errno($ch)) {
        $error = curl_error($ch);
        file_put_contents(__DIR__ . '/bot_error.log', date('Y-m-d H:i:s') . " - apiRequest curl error: {$error}\n", FILE_APPEND);
        curl_close($ch);
...
function getBotInfo($token) {
    ...
    if (curl_errno($ch)) {
        $error = curl_error($ch);
        file_put_contents(__DIR__ . '/bot_error.log', date('Y-m-d H:i:s') . " - getBotInfo curl error: {$error}\n", FILE_APPEND);
        curl_close($ch);
        return false;
    }
    ...
    file_put_contents(__DIR__ . '/bot_error.log', date('Y-m-d H:i:s') . " - getBotInfo response: {$result}\n", FILE_APPEND);
...
function setWebhook($token, $url) {
    ...
    if (curl_errno($ch)) {
        $error = curl_error($ch);
        file_put_contents(__DIR__ . '/bot_error.log', date('Y-m-d H:i:s') . " - setWebhook curl error: {$error}\n", FILE_APPEND);
        curl_close($ch);
        return ['ok' => false, 'error' => $error];
    }
...
function callGeminiAPI($messages, $system_prompt = '') {
    ...
    if (curl_errno($ch)) {
        $error = curl_error($ch);
        file_put_contents(__DIR__ . '/bot_error.log', date('Y-m-d H:i:s') . " - callGeminiAPI curl error: {$error}\n", FILE_APPEND);
        curl_close($ch);
        return "Xatolik yuz berdi: {$error}. Iltimos qaytadan urinib ko'ring.";
    }
    ...
    // Log error response
    file_put_contents(__DIR__ . '/bot_error.log', date('Y-m-d H:i:s') . " - callGeminiAPI error response: {$result}\n", FILE_APPEND);
